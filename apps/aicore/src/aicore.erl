...
apply_recommendations(Recommendations) ->
    case aicore_config:get_env_ensure(dry_run, true, boolean) of
        true ->
            ?LOG_INFO(#{event => dry_run_active, message => "'dry_run' is enabled - will not perform any actual scaling"}),
            lists:foreach(fun(ScaleTarget) -> 
                                  ?LOG_INFO(#{event => scale, message => io_lib:format("Will scale [~s] '~s' in namespace '~s' from ~p -> ~p", 
                                                                                       [
            ...
            ok;
        false ->
                ?LOG_INFO(#{event => performing_actions, message => "will proceed to perform necessary scale actions"}),
...
map_target(not_allowed, #scale_target{current_replicas = R} = Y) when R > 0 -> {true, Y#scale_target{desired_replicas = 0}};
map_target(not_allowed, #scale_target{current_replicas = 0}) -> false;
map_target(allowed, #scale_target{current_replicas = R}) when R > 0 -> false;
map_target(allowed, #scale_target{current_replicas = 0, desired_replicas = undefined} = Y) -> {true, Y#scale_target{desired_replicas = 1}};
map_target(allowed, #scale_target{current_replicas = 0, desired_replicas = Desired} = Y) -> {true, Y#scale_target{desired_replicas = Desired}}.
