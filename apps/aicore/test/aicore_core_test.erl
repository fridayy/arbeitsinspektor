...
scale_down_all(_) ->
    ...
    Defintion = aicore:bhd("mon-tue[09:00-18:00]", #{
        <<"leftshift.one/environment">> => <<"development">>
    }),
    Targets = [
        #scale_targets{
            targets = [
    ...
    ?_assertMatch(
        [
            #scale_target{current_replicas = 1, desired_replicas = 0},
            #scale_target{current_replicas = 1, desired_replicas = 0}
        ],
        R
    ).
...
scale_up_all(_) ->
    meck:expect(aicore_time, now_utc, fun() -> {{2022, 11, 21}, {7, 59, 59}} end),
    Defintion = aicore:bhd("mon-tue[07:00-18:00]", #{
        <<"leftshift.one/environment">> => <<"development">>
    }),
    Targets = [
        #scale_targets{
    ...
    ?_assertMatch(
        [
            #scale_target{current_replicas = 0, desired_replicas = 1},
            #scale_target{current_replicas = 0, desired_replicas = 1}
        ],
        R
    ).
...
respect_already_set_desired_replicas_for_scaleup(_) ->
    ...
    ?_assertMatch(
        [
            #scale_target{current_replicas = 0, desired_replicas = 3},
            #scale_target{current_replicas = 0, desired_replicas = 1}
        ],
        R
    ).
